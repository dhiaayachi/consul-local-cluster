...
docker exec  consul-client1 consul acl set-agent-token default $CONSUL_HTTP_TOKEN
docker exec  consul-client2 consul acl set-agent-token default $CONSUL_HTTP_TOKEN
...
docker exec -d consul-client1 consul connect proxy -sidecar-for counting -token $CONSUL_HTTP_TOKEN &
sleep 1s
...
docker exec -d consul-client2 consul connect proxy -sidecar-for dashboard -token $CONSUL_HTTP_TOKEN &
sleep 1s

# Create Consul intention with Dashboard as source and Counting as destination
docker exec -d consul-client1 consul intention create -token $CONSUL_HTTP_TOKEN dashboard counting
sleep 1s
